...
class LauncherDialog extends ModalDialog {
    _init(settings) {
        ...
        if (this._layout.entry) {
          ...
          // activate/cursor-movement - so returning Clutter.EVENT_STOP
            ...
            this._layout.entry.connect('key-press-event', (_actor, event) => this._onKeyPress(event));
        }
    ...
    _onKeyPress(event) {
        const symbol = event.get_key_val();
        const RET = typeof Clutter.KEY_Return !== 'undefined' ? Clutter.KEY_Return : 65293;
        const KP = typeof Clutter.KEY_KP_Enter !== 'undefined' ? Clutter.KEY_KP_Enter : 65421;
        const ESC = typeof Clutter.KEY_Escape !== 'undefined' ? Clutter.KEY_Escape : 65307;
        const DOWN = typeof Clutter.KEY_Down !== 'undefined' ? Clutter.KEY_Down : 65364;
        const UP = typeof Clutter.KEY_Up !== 'undefined' ? Clutter.KEY_Up : 65362;
        log('gnome-launcher: key=' + symbol + ', RET=' + RET + ', KP=' + KP + ', ESC=' + ESC + ', DOWN=' + DOWN + ', UP=' + UP);
