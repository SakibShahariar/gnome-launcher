...
class LauncherDialog extends ModalDialog {
    ...
    _applyPosition() {
        ...
        let [, naturalWidth] = this.dialogLayout.get_preferred_width(-1);
        let [, naturalHeight] = this.dialogLayout.get_preferred_height(-1);
        // Guard against 0 on first frame before Clutter allocation
        if (naturalWidth < 1)
            naturalWidth = this._layout.fixedWidth || 460;
        if (naturalHeight < 1)
            naturalHeight = 400;
        const margin = 32;
