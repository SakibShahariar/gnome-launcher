...
//
// Search row: soft glass over the banner (follows global opacity).
...
import {withOpacity, withOpacityText, withSelectedOpacity, resolveWallpaperUri, platedAppIcon} from '../theme.js';
...
export class HeroBannerLayout {
    ...
    buildUI() {
        ...
        let bannerStyle = 'border-radius: 20px 20px 0 0; padding: 14px 16px 20px 16px; spacing: 14px;';
        if (wallpaperUri) {
        ...
        // and don't overlap the first result (image shows cramped/overlap at 128-148)
        this.banner.set_height(160);

        // Soft glass over the banner wallpaper — follow global opacity,
        // with a light floor so text stays readable without going solid.
        const searchOpacity = Math.min(Math.max(this.theme.opacity, 0.35), 0.55);
        const searchRow = new St.BoxLayout({
            x_expand: true,
            style: `background-color: ${withOpacity(this.theme.surface, searchOpacity)}; ` +
                   'border-radius: 16px; padding: 8px 14px; ' +
                   `border: 1px solid ${withOpacity(this.theme.outline_variant, 0.45)};`,
        });
        ...
        for (const [modeId, iconName, , enabled] of MODES) {
            ...
            const circle = new St.Bin({
                reactive: enabled,
                style: this._circleStyle(active, enabled),
            });
        ...
        // Hard spacer guarantees gap even if banner height/clipping varies
        const bannerSpacer = new St.Widget({style: 'height: 14px; background-color: transparent;'});
        card.add_child(bannerSpacer);

        this.resultsBox = new St.BoxLayout({
            vertical: true,
            style: 'padding: 8px 8px 10px; spacing: 2px;',
        });
    ...
    _circleStyle(active, enabled) {
        if (active)
            return `background-color: ${withSelectedOpacity(this.theme.primary, this.theme.opacity)}; border-radius: 999px; padding: 10px;`;
        return `background-color: ${withOpacity(this.theme.surface, 0.55)}; ` +
    ...
    _switchMode(modeId) {
        ...
        for (const {modeId: id, circle, icon} of this._modeCircles) {
            ...
            circle.style = this._circleStyle(active, enabled);
    ...
    _renderApps() {
        ...
        apps.forEach((app, localIndex) => {
            ...
            const icon = platedAppIcon(app.get_icon(), Math.round(16 * this.theme.scale), this.theme.surface, {platesEnabled: this.theme.appIconPlates, focused: selected, extraStyle: 'margin-right: 10px;'});
    ...
    _renderRunPreview() {
        ...
        const icon = new St.Icon({icon_name: 'utilities-terminal-symbolic', icon_size: Math.round(16 * this.theme.scale), style: `margin-right: 10px; color: ${iconColor};`});
        const label = new St.Label({
    ...
    _renderWindowResults() {
        ...
        this._modeResults.slice(0, MAX_RESULTS).forEach((item, index) => {
            ...
            const iconColor = selected
                ...
                : this.theme.on_surface_variant;
            const icon = item.app
                ? platedAppIcon(item.app.get_icon(), Math.round(16 * this.theme.scale), this.theme.surface, {platesEnabled: this.theme.appIconPlates, focused: true, extraStyle: 'margin-right: 10px;'})
