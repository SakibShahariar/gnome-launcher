...
//
// Search row: transparent + backdrop blur via Blur.BlurEffect (blur-effect pkg).
// St's CSS doesn't support backdrop-filter/blur(), but Blur.BlurEffect
...
export class HeroBannerLayout {
    ...
    buildUI() {
        ...
        const searchRow = new St.BoxLayout({
            x_expand: true,
            style: `background-color: alpha(${this.theme.surface}, 0.70); ` +
                   'border-radius: 20px; padding: 8px 14px; ' +
                   'border: 1px solid alpha(#ffffff, 0.12);',
        });
        searchRow.set_clip_to_allocation(false);
        // Backdrop blur — Blur.BlurEffect (from blur-effect-1.0) blurs behind the actor
        // Props per Blur-1.0.gir: radius (int), brightness (float 0-1), mode (BACKGROUND/ACTOR)
        // Fallback stays semi-opaque (0.70) if blur unavailable
        try {
            // Use imports.gi for optional Blur (static `import Blur from 'gi://Blur'` would hard-fail if typelib missing)
            const Blur = imports.gi.Blur;
            const blurEffect = new Blur.BlurEffect({
                radius: 30,
                brightness: 0.9,
            ...
            // Blur succeeded — make it more translucent so wallpaper shows through
            searchRow.style = `background-color: alpha(${this.theme.surface}, 0.38); ` +
                              'border-radius: 20px; padding: 8px 14px; ' +
