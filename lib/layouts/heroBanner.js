...
export class HeroBannerLayout {
    ...
    buildUI() {
        ...
        const banner = new St.BoxLayout({
            vertical: true,
            style: `background-color: ${withOpacity(this.theme.primary_container, this.theme.opacity)}; ` +
        ...
        for (const [modeId, iconName, , enabled] of MODES) {
            ...
            const circle = new St.Bin({
                ...
                style: active
                    ...
                    : `background-color: ${withOpacity(this.theme.on_primary_container, 0.15)}; ` +
                      `border-radius: 999px; padding: 8px;${enabled ? '' : ' opacity: 0.5;'}`,
            });
