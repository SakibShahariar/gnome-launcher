...
export class SidebarLayout {
    ...
    _switchMode(modeId) {
        ...
        for (const {modeId: id, iconBox, icon} of this._railIcons) {
            ...
            iconBox.style = active
                ? `background-color: ${this.theme.primary}; border-radius: 12px; padding: 8px;`
                : 'border-radius: 12px; padding: 8px;';
            icon.style = `color: ${active ? this.theme.on_primary : this.theme.on_surface_variant};`;
        }
    ...
    _renderApps() {
        ...
        apps.forEach((app, localIndex) => {
            ...
            const icon = new St.Icon({gicon: app.get_icon(), icon_size: Math.round(18 * this.theme.scale), style: 'margin-right: 10px;'});
    ...
    _renderModeResults() {
        ...
        if (this._modeResults.length === 0) {
            const emptyLabel = new St.Label({
                text: this.mode === 'files' ? 'Type to search files' : 'Nothing here yet',
                style: `color: ${this.theme.muted}; font-size: 12px; padding: 10px 12px;`,
            });
        ...
        this._modeResults.slice(0, MAX_RESULTS).forEach((item, index) => {
            ...
            const icon = new St.Icon({icon_name: iconName, icon_size: Math.round(18 * this.theme.scale), style: 'margin-right: 10px;'});
            const label = new St.Label({
                text: item.name,
                y_align: Clutter.ActorAlign.CENTER,
                style: `color: ${selected ? this.theme.on_primary : this.theme.on_surface_variant}; font-size: 13px;`,
