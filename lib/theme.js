 ...
 * (primary) chip. Applied to every app icon (not just focused) so tile
 * geometry never shifts between selected/unselected states.
 *
 * Pass {platesEnabled: false} to draw the bare icon instead.
 */
export function platedAppIcon(gicon, iconSize, surfaceColor, {platesEnabled = true, align, extraStyle} = {}) {
    ...
    const plate = new St.BoxLayout({
        style: `background-color: ${surfaceColor}; border-radius: ${radius}px; padding: ${pad}px;${extraStyle ? ' ' + extraStyle : ''}`,
        x_align: xAlign,
        y_align: Clutter.ActorAlign.CENTER,
    });
