...
/**
 * Wraps a colored app icon in a small rounded "plate" of the surface
 * color so it stays legible when the tile background becomes the accent
 * (primary) chip. Plate geometry is always reserved on every tile so
 * nothing shifts when focus moves; the surface background is only
 * painted on the focused tile (pass {focused: false} elsewhere).
 *
 * Pass {platesEnabled: false} to draw the bare icon instead.
 */
export function platedAppIcon(gicon, iconSize, surfaceColor, {platesEnabled = true, align, extraStyle, focused = true} = {}) {
    const xAlign = align ?? Clutter.ActorAlign.CENTER;
    if (!platesEnabled) {
        ...
        if (extraStyle)
            icon.set_style(extraStyle);
        return icon;
    }
    const pad = Math.max(3, Math.round(iconSize * 0.22));
    const radius = Math.max(4, Math.round(iconSize * 0.30));
    const icon = new St.Icon({
        gicon,
        icon_size: iconSize,
    ...
    const plate = new St.BoxLayout({
        style: `background-color: ${focused ? surfaceColor : 'transparent'}; border-radius: ${radius}px; padding: ${pad}px;${extraStyle ? ' ' + extraStyle : ''}`,
        x_align: xAlign,
        y_align: Clutter.ActorAlign.CENTER,
    });
